...
Recent progress
- 2025-12-28: Phase 3 slice: allow one-finger pan/scroll while in “add text annotation” mode (so “pinch zoom → pan around to inspect the note” works without forcing a mode exit). Implemented by treating `ReaderMode.ADDING_TEXT_ANNOT` like `VIEWING/SEARCHING` for scroll routing in `platform/android/src/org/opendroidpdf/app/reader/gesture/ReaderGestureController.java`. Build: `cd platform/android && ./gradlew testDebugUnitTest assembleDebug -x lint` (**PASS**). Smokes: `scripts/geny_smoke.sh`, `scripts/geny_epub_smoke.sh` (**PASS**). Commit: `1d0d5a4f`.
- 2025-12-28: Zoom pan crash: fixed a `StackOverflowError` triggered by “pinch zoom → pause → one-finger drag” due to an accidental recursive call in `ReaderViewGestureHost`’s `getScrollBoundsForView` bridge (now correctly calls `ReaderView.this.getScrollBoundsForView(v)`). Updated `scripts/geny_pinch_zoom_smoke.sh` to reproduce the exact interaction pattern (progressive zoom-in, sleep, then drag/pan) and fail the smoke if UIAutomator reports failures. Build: `cd platform/android && ./gradlew testDebugUnitTest assembleDebug -x lint` (**PASS**). Smokes: `scripts/geny_smoke.sh`, `scripts/geny_pinch_zoom_smoke.sh` (**PASS**). Commit: `54c7f343`.
- 2025-12-28: Release: pushed `1.3.60 (121)` to the self-hosted F-Droid repo. Verified server-side: `versionName=1.3.60 versionCode=121 apk=org.opendroidpdf_121.apk`. Commit: `4827fa81`.
- 2025-12-30: Release: pushed `1.3.61 (122)` to the self-hosted F-Droid repo. Verified server-side: `versionName=1.3.61 versionCode=122 apk=org.opendroidpdf_122.apk`. Commit: `f6bf139a`.
- 2025-12-28: Release: pushed `1.3.59 (120)` to the self-hosted F-Droid repo. Verified server-side: `versionName=1.3.59 versionCode=120 apk=org.opendroidpdf_120.apk`. Commit: `0acaccd3`.
- 2025-12-28: Crash diagnostics: added a persistent app-owned log (`cache/tmpfiles/opendroidpdf_app.log`) + uncaught-exception crash report (`cache/tmpfiles/opendroidpdf_last_crash.txt`) with a one-tap share prompt on next launch. Added `scripts/geny_release_crash_watch_smoke.sh` to install the signed release APK and verify “open via DocumentsUI + wait” without crashes. Build: `cd platform/android && ./gradlew testDebugUnitTest assembleDebug -x lint` (**PASS**). Smokes: `scripts/geny_smoke.sh`, `scripts/geny_epub_smoke.sh`, `scripts/geny_pdf_text_annot_smoke.sh`, `scripts/geny_release_crash_watch_smoke.sh` (**PASS**). Commit: `c32a97ba`.
- 2025-12-28: Release: pushed `1.3.58 (119)` to the self-hosted F-Droid repo. Verified server-side: `versionName=1.3.58 versionCode=119 apk=org.opendroidpdf_119.apk`. Commit: `5f5fdc6d`.
...
- 2025-12-28: Zoom stability: prevented device-specific crashes when progressively pinch-zooming by aborting in-flight HQ patch renders when the target bitmap is not currently displayed (prevents concurrent native renders into the 2-bitmap pool). Added UIAutomator-based regression `scripts/geny_pinch_zoom_smoke.sh`. Build: `cd platform/android && ./gradlew testDebugUnitTest assembleDebug -x lint` (**PASS**). Smokes: `scripts/geny_smoke.sh`, `scripts/geny_epub_smoke.sh`, `scripts/geny_pdf_text_annot_smoke.sh`, `scripts/geny_pinch_zoom_smoke.sh` (**PASS**). Commit: `b3f0ae4d`.
