...
Recent progress
- 2025-12-25: Phase 5 slice: moved shared data-holder types (`Hit`, `PatchInfo`, `TextProcessor`) into `:core` (from `platform/android/src/org/opendroidpdf/`) to clarify module ownership and keep the app layer UI-focused. Commit: `97417311`. Build: `cd platform/android && ./gradlew testDebugUnitTest assembleDebug -x lint` (**PASS**). Smokes: `scripts/geny_smoke.sh`, `scripts/geny_epub_smoke.sh` (**PASS**).
