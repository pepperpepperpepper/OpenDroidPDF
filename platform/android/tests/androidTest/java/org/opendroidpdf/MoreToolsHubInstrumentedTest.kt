...
class MoreToolsHubInstrumentedTest {

    @Test
    fun quickActionMoreTools_opensMoreToolsHubSheet() {
        ...
        ActivityScenario.launch<OpenDroidPDFActivity>(intent).use { scenario ->
            ...
            onView(withId(R.id.quick_action_more_tools)).perform(click())
            onView(withId(R.id.more_tools_hub_root)).check(matches(isDisplayed()))
        }
    }
