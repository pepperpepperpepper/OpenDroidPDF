...
class NavigationMenuInstrumentedTest {

    @Test
    fun navigationMenu_openThumbnails_navigatesToPage() {
        ...
        ActivityScenario.launch<OpenDroidPDFActivity>(intent).use { scenario ->
            ...
            onView(withId(R.id.navigation_menu_sheet_root)).check(matches(isDisplayed()))
            onView(withId(R.id.navigation_menu_action_thumbnails)).perform(click())
            onView(withId(R.id.thumbnails_recycler)).check(matches(isDisplayed()))

            val page2Desc = context.getString(R.string.thumbnail_item_content_description, 2, 2)
            onView(withContentDescription(page2Desc)).perform(click())
            assertTrue("Expected to be on page 1 after selecting thumbnail 2", waitForPage(scenario, 1))
        }
    }
