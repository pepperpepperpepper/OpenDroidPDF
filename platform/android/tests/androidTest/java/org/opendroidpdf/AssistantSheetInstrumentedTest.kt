...
class AssistantSheetInstrumentedTest {

    @Test
    fun navigateViewSheet_assistantRowOpensAssistantSheet() {
        ...
        ActivityScenario.launch<OpenDroidPDFActivity>(intent).use { scenario ->
            ...
            onView(withId(R.id.navigate_view_action_assistant)).perform(click())
            onView(withId(R.id.assistant_sheet_root)).check(matches(isDisplayed()))
        }
    }
