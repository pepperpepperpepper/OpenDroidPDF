...
# Genymotion smoke for "XFA forms unsupported" banner:
# - Generate a tiny PDF that includes an AcroForm /XFA entry (MuPDF doesn't support XFA)
# - Open it via DocumentsUI (content:// URI)
# - Assert an informational banner appears and "Learn more" shows the explainer dialog
#
# Usage:
#   DEVICE=localhost:<port> APK=/path/to/OpenDroidPDF-debug.apk ./scripts/geny_pdf_xfa_banner_smoke.sh

DEVICE="${DEVICE:-${GENYMOTION_DEV:-${ANDROID_SERIAL:-}}}"
APK=${APK:-/mnt/subtitled/opendroidpdf-android-build/outputs/apk/debug/OpenDroidPDF-debug.apk}
PDF_REMOTE_PATH=${PDF_REMOTE_PATH:-/sdcard/Download/odp_xfa_banner_smoke.pdf}
...
echo "[4/4] OK"
echo "OK: XFA unsupported banner + explainer appeared"
