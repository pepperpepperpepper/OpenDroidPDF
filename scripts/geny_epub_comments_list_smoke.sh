...
# Genymotion smoke for "Comments list + next/prev navigation (EPUB / sidecar)":
# - Open an EPUB via DocumentsUI (sidecar-backed session)
# - Create 2 sidecar text notes at distinct vertical positions
# - Open Comments list, tap an entry to jump/select
# - Use Next/Previous comment navigation and assert the selection box moves
#
# Usage:
#   DEVICE=localhost:<port> APK=/path/to/OpenDroidPDF-debug.apk ./scripts/geny_epub_comments_list_smoke.sh

DEVICE="${DEVICE:-${GENYMOTION_DEV:-${ANDROID_SERIAL:-}}}"
APK=${APK:-/mnt/subtitled/opendroidpdf-android-build/outputs/apk/debug/OpenDroidPDF-debug.apk}
EPUB_LOCAL=${EPUB_LOCAL:-test_assets/hello.epub}
EPUB_REMOTE_PATH=${EPUB_REMOTE_PATH:-/sdcard/Download/odp_comments_list_smoke.epub}
TOKEN1=${TOKEN1:-ODPEPUBNOTE1}
TOKEN2=${TOKEN2:-ODPEPUBNOTE2}
...
echo "[3/6] Push fixture EPUB to Downloads"
adb -s "$DEVICE" push "$EPUB_LOCAL" "$EPUB_REMOTE_PATH" >/dev/null
fname="$(basename "$EPUB_REMOTE_PATH")"

echo "[4/6] Open EPUB via DocumentsUI"
_open_epub_via_documentsui "$fname"
sleep 1.0

echo "[5/6] Create two notes"
read -r w h < <(_wm_size)
cx=$((w * 5 / 10))
y_top=$((h * 30 / 100))
y_bot=$((h * 70 / 100))
_add_text_note_at "$cx" "$y_top" "$TOKEN1"
_add_text_note_at "$cx" "$y_bot" "$TOKEN2"
...
shot1="$(mktemp -t odp_epub_comments1_XXXXXX).png"
shot2="$(mktemp -t odp_epub_comments2_XXXXXX).png"
shot3="$(mktemp -t odp_epub_comments3_XXXXXX).png"
